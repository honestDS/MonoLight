...
async def test_prompts_management_full_flow():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
        ...
        r_c_type = await ac.post("/api/v1/prompts/create", json={"name": "p1", "content": {"text": "hello"}}, headers=headers)
        assert r_c_type.status_code == 422

        # 1. 正常创建
        r_ok = await ac.post("/api/v1/prompts/create", json={"name": "p1", "content": "hi"}, headers=headers)
        pid = r_ok.json()["data"]["id"]

        # --- 422 类型错误验证：update 接口 prompt_id 类型错误 ---
        r_up_type = await ac.post("/api/v1/prompts/update", params={"prompt_id": "not_an_id"}, json={"name": "new"}, headers=headers)
