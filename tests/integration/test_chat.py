...
async def test_full_chat_audit_flow():
    async with AsyncClient(
        ...
        await ac.post(
            "/api/v1/profiles/activate", params={"profile_id": 1}, headers=headers
        )
                        ...
                        "tool_calls": [
                            {
                                ...
                                "function": {
                                    "name": "execute_shell",
                                    "arguments": '{"command":"rm risky_file"}',
                                },
                            }
                        ],
                    }
            ...
            "choices": [
                {
                    "message": {
                        "role": "assistant",
                        "content": "Command requires confirmation.",
                    }
                }
            ]
        }
        ...
        with patch(
            ...
            AsyncMock(side_effect=[mock_llm_call, mock_llm_final]),
        ):
            with patch(
                "app.core.dispatcher.audit_command",
                AsyncMock(return_value=mock_audit_res),
            ):
                resp = await ac.post(
                    "/api/v1/chat/completions",
                    json={"message": "delete it"},
                    headers=headers,
                )
        ...
        with patch(
            "app.core.dispatcher.ShellExecutor.execute",
            AsyncMock(return_value=json.dumps({"stdout": "deleted", "exit_code": 0})),
        ) as mock_shell:
            with patch(
                "app.providers.llm.client.LLMClient.generate",
                AsyncMock(
                ...
                resp = await ac.post(
                    "/api/v1/chat/completions",
                    json={"message": confirm_command},
                    headers=headers,
                )
                assert resp.status_code == 200
                assert (
                    "Task completed" in resp.json()["choices"][0]["message"]["content"]
                )
...
async def test_chat_validation_errors():
    async with AsyncClient(
        ...
        headers = {"Authorization": f"Bearer {token}"}
        r = await ac.post(
            "/api/v1/chat/completions", json={"message": 123}, headers=headers
        )
