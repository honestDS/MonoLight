...
async def test_providers_workflow(client, db_session, test_user_token):
    ...
    res_create = await client.post("/api/v1/providers/create", json=create_payload, headers=headers)
    assert res_create.status_code == 200
    provider_id = res_create.json()["data"]["id"]
    
    # 3. 测试获取详情 (GET /api/v1/providers/get?provider_id=X)
    res_get = await client.get(f"/api/v1/providers/get?provider_id={provider_id}", headers=headers)
    ...
    res_update = await client.post(f"/api/v1/providers/update?provider_id={provider_id}", json=update_payload, headers=headers)
    assert res_update.status_code == 200
    assert res_update.json()["data"]["name"].startswith("Updated_")

    # 4. 测试删除 (POST /api/v1/providers/delete?provider_id=X)
    res_del = await client.post(f"/api/v1/providers/delete?provider_id={provider_id}", headers=headers)
    assert res_del.status_code == 200
    
    
    # 5. 验证已删除：统一处理可能抛出的业务异常
    try:
        res_final = await client.get(f"/api/v1/providers/get?provider_id={provider_id}", headers=headers)
