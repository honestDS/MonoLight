...
with patch.object(chromadb, "PersistentClient", ImportSafePersistentClient):
    ...
    from app.core.memory import MemoryConflictError, submit_memory_cleanup_retry
...
@pytest.mark.asyncio
async def test_cleanup_retry_rejects_store_pointing_to_missing_cleanup_job(memory_session_factory: Any) -> None:
    uid = "cleanup-retry-missing-job"
    ...
    async with memory_session_factory() as db:
        updated = await memory_store_crud.update_by_uid(
            db,
            uid=uid,
            old_collection_name="missing-job-old-collection",
            old_collection_cleanup_status=LongTermMemoryOldCollectionCleanupStatus.FAILED,
            old_collection_cleanup_job_id=999_999,
            commit=True,
        )
    ...
    async with memory_session_factory() as db:
        with pytest.raises(MemoryConflictError):
            await submit_memory_cleanup_retry(
                db,
                uid=uid,
                job_id=999_999,
                dedupe_key="cleanup-retry-missing-job",
            )
...
async def test_reindex_cleanup_retry_after_switch(
    ...
    uid = "memory_cleanup_retry-cleanup-reindex"
    source_collection = "memory_cleanup_retry-cleanup-reindex-source"
    target_collection = "memory_cleanup_retry-cleanup-reindex-target"
    ...
    await configure_store(
        ...
        uid=uid,
        channel_id=11,
        model_id="memory_cleanup_retry-reindex-model",
        dimensions=3,
        signature="memory_cleanup_retry-reindex-signature",
    ...
    original_job = await claim_job(
        ...
        dedupe_key="memory_cleanup_retry-cleanup-reindex-original",
        owner="memory_cleanup_retry-cleanup-reindex-owner",
        payload=payload,
    )
    ...
    async with memory_session_factory() as db:
        first = await submit_memory_cleanup_retry(
            ...
            uid=uid,
            job_id=original_job_id,
            dedupe_key="memory_cleanup_retry-cleanup-reindex-retry",
        )
    ...
    claimed = await claim_job(
        ...
        operation=LongTermMemoryMutationOperation.REINDEX,
        job_id=retry_job_id,
        owner="memory_cleanup_retry-cleanup-reindex-worker",
    )
    ...
    result = await executor.execute_claimed(claimed, "memory_cleanup_retry-cleanup-reindex-worker")
    ...
    assert store.active_embedding_model_id == "memory_cleanup_retry-reindex-model"
    assert store.active_embedding_dimensions == 3
    assert store.active_embedding_signature == "memory_cleanup_retry-reindex-signature"
...
async def test_reindex_cleanup_retry_after_expired_switch_failure(
    ...
    uid = "memory_cleanup_retry-cleanup-reindex-expired"
    source_collection = "memory_cleanup_retry-cleanup-reindex-expired-source"
    target_collection = "memory_cleanup_retry-cleanup-reindex-expired-target"
    embedding_revision = 4
    target_index_revision = 8
    await configure_store(
        memory_session_factory,
    ...
    original_job = await claim_job(
        ...
        uid=uid,
        operation=LongTermMemoryMutationOperation.REINDEX,
        dedupe_key="memory_cleanup_retry-cleanup-reindex-expired-original",
        owner="memory_cleanup_retry-cleanup-reindex-expired-owner",
        payload=payload,
        max_attempts=1,
    )
    assert original_job is not None
    ...
    async with memory_session_factory() as db:
        submission = await submit_memory_cleanup_retry(
            ...
            uid=uid,
            job_id=original_job_id,
            dedupe_key="memory_cleanup_retry-cleanup-reindex-expired-retry",
        )
    assert submission.created
...
async def test_migration_cleanup_retry_after_cancellation(
    ...
    uid = "memory_cleanup_retry-cleanup-migration"
    source_collection = "memory_cleanup_retry-cleanup-migration-source"
    target_collection = "memory_cleanup_retry-cleanup-migration-target"
    source = {
        "channel_id": 21,
        "model_id": "memory_cleanup_retry-migration-source-model",
        "dimensions": 3,
        "signature": "memory_cleanup_retry-migration-source-signature",
        "collection": source_collection,
        "revision": 5,
    }
    target = {
        "channel_id": 22,
        "model_id": "memory_cleanup_retry-migration-target-model",
        "dimensions": 4,
        "signature": "memory_cleanup_retry-migration-target-signature",
        "collection": target_collection,
        "revision": 6,
    }
    ...
    original_job = await _create_migration_job(
        ...
        source=source,
        target=target,
        dedupe_key="memory_cleanup_retry-cleanup-migration-original",
    )
    assert original_job.id is not None
    original_job_id = original_job.id
    async with memory_session_factory() as db:
    ...
    async with memory_session_factory() as db:
        submission = await submit_memory_cleanup_retry(
            ...
            uid=uid,
            job_id=original_job_id,
            dedupe_key="memory_cleanup_retry-cleanup-migration-retry",
        )
    assert submission.created
    assert submission.job.id is not None
    ...
    claimed = await claim_job(
        ...
        operation=LongTermMemoryMutationOperation.EMBEDDING_MIGRATION,
        job_id=retry_job_id,
        owner="memory_cleanup_retry-cleanup-migration-worker",
    )
    assert claimed is not None
    executor = MemoryJobExecutor(
        {
    ...
    result = await executor.execute_claimed(claimed, "memory_cleanup_retry-cleanup-migration-worker")
