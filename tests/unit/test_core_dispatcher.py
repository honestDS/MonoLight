...
async def test_audit_tool_call_no_audit_config(mock_db, mock_audit_profile):
    ...
    res = await ChatDispatcher._audit_tool_call(db, mock_audit_profile, "execute_shell", {"command": "ls"}, [])
    assert res is None
...
async def test_audit_tool_call_high_risk_blocked(mock_db, mock_audit_profile):
    ...
    with patch("app.core.dispatcher.audit_command", AsyncMock(return_value=mock_audit_res)):
        res_json = await ChatDispatcher._audit_tool_call(db, mock_audit_profile, "execute_shell", {"command": "rm -rf /"}, [])
...
async def test_audit_tool_call_confirmation_required(mock_db, mock_audit_profile):
    ...
    with patch("app.core.dispatcher.audit_command", AsyncMock(return_value=mock_audit_res)):
        res_json = await ChatDispatcher._audit_tool_call(db, mock_audit_profile, "execute_shell", {"command": "rm test.txt"}, [])
        res = json.loads(res_json)
...
async def test_audit_tool_call_token_validation_success(mock_db, mock_audit_profile):
    db, _ = mock_db
    messages = [{"role": "tool", "content": json.dumps({"error": "confirmation_required", "reason": "..."})}]
    command = f"{CONFIRMATION_TOKEN} rm test.txt"
    res = await ChatDispatcher._audit_tool_call(db, mock_audit_profile, "execute_shell", {"command": command}, messages)
    assert res is None
...
async def test_audit_tool_call_malicious_token_injection(mock_db, mock_audit_profile):
    ...
    with patch("app.core.dispatcher.audit_command", AsyncMock(return_value=mock_audit_res)) as mock_audit:
        res_json = await ChatDispatcher._audit_tool_call(db, mock_audit_profile, "execute_shell", {"command": command}, messages)
        res = json.loads(res_json)
        assert res["error"] == "confirmation_required"
...
async def test_dispatch_flow_audit_interception(mock_db, mock_audit_profile):
    ...
    mock_llm_response_1 = {"choices": [{"message": {"role": "assistant", "tool_calls": [{"id": "t1", "type": "function", "function": {"name": "execute_shell", "arguments": '{"command":"rm file"}'}}]}}]}
    mock_llm_response_2 = {"choices": [{"message": {"role": "assistant", "content": "I need your confirmation"}}]}
    with patch("app.core.context.ContextManager.get_messages", AsyncMock(return_value=[])):
        with patch("app.providers.llm.client.LLMClient.generate", AsyncMock(side_effect=[mock_llm_response_1, mock_llm_response_2])):
            with patch("app.core.dispatcher.audit_command", AsyncMock(return_value={"score": 6, "reason": "risky"})):
                result = await ChatDispatcher.dispatch(db, "delete file", "u1")
                assert "I need your confirmation" in result["choices"][0]["message"]["content"]
