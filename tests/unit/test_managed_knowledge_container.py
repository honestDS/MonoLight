...
async def memory_database(tmp_path) -> AsyncIterator[async_sessionmaker[AsyncSession]]:
    database_path = tmp_path / "managed-knowledge-managed_knowledge_container.db"
    ...
    *,
    ...
    async with session_factory() as db:
        channel = ModelChannel(
            name="managed_knowledge_container-embedding",
        ...
        library = PromptLibrary(name="managed_knowledge_container-prompts", uid="user-1", content="prompt")
        db.add(library)
        await db.flush()
        profile = Profile(name="Managed Knowledge Profile", uid="user-1", prompt_id=library.id, configs={})
        db.add(profile)
        await db.flush()
...
async def test_first_write_creates_managed_container_binding_and_first_job(
    ...
    profile, channel = await _create_profile_runtime(memory_database)
    ...
    result = await _submit_profile_create(
        ...
        dedupe_key="managed_knowledge_container-first-write",
    )
...
async def test_first_write_container_binding_and_item_remain_invisible_until_submission_commit(
    ...
    profile, _channel = await _create_profile_runtime(memory_database)
    async with memory_database() as writer:
        result = await knowledge_job_manager.submit_create_for_profile(
            ...
            source_type=ManagedKnowledgeSourceType.LLM_TOOL,
            actor=ManagedKnowledgeActorType.LLM,
            dedupe_key="managed_knowledge_container-atomic-first-write",
            commit=False,
        )

        async with memory_database() as observer:
...
async def test_first_write_without_memory_runtime_leaves_no_empty_container(
    ...
    profile, _channel = await _create_profile_runtime(memory_database, create_store=False)
    ...
    with pytest.raises(ManagedKnowledgeRuntimeUnavailableError):
        await _submit_profile_create(
            ...
            dedupe_key="managed_knowledge_container-no-runtime",
        )
...
async def test_first_write_with_disabled_embedding_runtime_leaves_no_empty_container(
    ...
    profile, _channel = await _create_profile_runtime(memory_database, channel_active=False)
    ...
    with pytest.raises(ManagedKnowledgeRuntimeUnavailableError):
        await _submit_profile_create(
            ...
            dedupe_key="managed_knowledge_container-disabled-runtime",
        )
...
async def test_first_write_rejects_memory_runtime_change_during_lazy_create(
    ...
    with pytest.raises(ManagedKnowledgeContainerConflictError):
        await _submit_profile_create(
            ...
            dedupe_key="managed_knowledge_container-runtime-changed",
        )
...
async def test_first_write_failure_rolls_back_container_binding_and_job(
    ...
    with pytest.raises(ManagedKnowledgeValidationError):
        await _submit_profile_create(
            ...
            content="   ",
            dedupe_key="managed_knowledge_container-invalid-first-write",
        )
...
async def test_concurrent_first_writes_converge_to_one_managed_container(
    ...
    first, second = await asyncio.gather(
        _submit_profile_create(
            ...
            dedupe_key="managed_knowledge_container-concurrent-one",
        ),
        _submit_profile_create(
            memory_database,
            profile_id=profile.id,
            ...
            dedupe_key="managed_knowledge_container-concurrent-two",
        ),
    )
...
async def test_write_after_managed_container_delete_creates_new_identifier_and_time(
    ...
    first = await _submit_profile_create(
        ...
        dedupe_key="managed_knowledge_container-before-delete",
    )
    ...
    second = await _submit_profile_create(
        ...
        dedupe_key="managed_knowledge_container-after-delete",
    )
