...
async def _fail_migration(
    ...
    assert job.id is not None
    owner = "memory_management_migration-migration-failure-worker"
...
async def test_memory_settings_expose_active_target_progress_capacity_and_cleanup(
    ...
    uid = "memory_management_migration-settings-owner"
    source, target = _migration_configs("memory_management_migration-settings")
    ...
    job = await _create_migration_job(
        ...
        source=source,
        target=target,
        dedupe_key="memory_management_migration-settings-migration",
    )
    assert job.id is not None
    async with memory_session_factory() as db:
        updated = await memory_store_crud.update_by_uid(
            ...
            old_collection_name="memory_management_migration-settings-old",
    ...
    assert result["old_collection_cleanup"]["name"] == "memory_management_migration-settings-old"
...
async def test_migration_list_and_detail_isolate_uid(memory_session_factory) -> None:
    owner = "memory_management_migration-migration-owner"
    other = "memory_management_migration-migration-other"
    owner_source, owner_target = _migration_configs("memory_management_migration-owner")
    other_source, other_target = _migration_configs("memory_management_migration-other")
    await configure_store(
        memory_session_factory,
        uid=owner,
    ...
    owner_job = await _create_migration_job(
        ...
        source=owner_source,
        target=owner_target,
        dedupe_key="memory_management_migration-owner-migration",
    )
    await _create_migration_job(
        ...
        source=other_source,
        target=other_target,
        dedupe_key="memory_management_migration-other-migration",
    )
...
async def test_migration_history_uses_revision_progress_when_store_tracks_newer_job(memory_session_factory) -> None:
    uid = "memory_management_migration-migration-history-owner"
    source, old_target = _migration_configs("memory_management_migration-migration-history-old")
    _current_source, current_target = _migration_configs("memory_management_migration-migration-history-current")
    ...
    old_job = await _create_migration_job(
        ...
        source=source,
        target=old_target,
        dedupe_key="memory_management_migration-migration-history-old",
    )
    assert old_job.id is not None

    historical_result = {"count": 13, "success_count": 9}
    old_owner = "memory_management_migration-migration-history-old-worker"
    ...
    current_job = await _create_migration_job(
        ...
        dedupe_key="memory_management_migration-migration-history-current",
    )
...
async def test_failed_migration_retry_creates_new_job_revision_target_and_preserves_active(
    ...
    uid = "memory_management_migration-migration-retry-owner"
    source, target = _migration_configs("memory_management_migration-retry")
    await configure_store(
        memory_session_factory,
        uid=uid,
        channel_id=source["channel_id"],
    ...
    old_job = await _create_migration_job(
        ...
        source=source,
        target=target,
        dedupe_key="memory_management_migration-migration-retry-original",
    )
...
async def test_switching_migration_cancellation_is_rejected(
    ...
    uid = "memory_management_migration-migration-switching"
    source, target = _migration_configs("memory_management_migration-switching")
    await configure_store(
        memory_session_factory,
        uid=uid,
        channel_id=source["channel_id"],
    ...
    job = await _create_migration_job(
        ...
        source=source,
        target=target,
        dedupe_key="memory_management_migration-switching-migration",
    )
    assert job.id is not None
    async with memory_session_factory() as db:
        updated = await memory_store_crud.update_by_uid(
