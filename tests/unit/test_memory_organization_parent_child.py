...
async def db_session(tmp_path) -> AsyncGenerator[AsyncSession]:
    database_path = tmp_path / "memory-organization-parent-child-memory_organization_parent_child.db"
    ...
    try:
        async with session_factory() as session:
            session.add(
                ModelChannel(
                    id=7,
                    name="memory-organization-parent-child-memory_organization_parent_child-channel",
