...
from tests.unit.memory_test_support import MemoryVectorBackend, claim_job, configure_store
...
def _migration_configs(
    *,
    ...
    source = {
        "channel_id": 1,
        "model_id": "memory_maintenance_lifecycle-source-model",
        "dimensions": 3,
        "signature": "memory_maintenance_lifecycle-source-signature",
        "collection": source_collection,
        "revision": 1,
    }
    target = {
        "channel_id": 2,
        "model_id": "memory_maintenance_lifecycle-target-model",
        "dimensions": 4,
        "signature": "memory_maintenance_lifecycle-target-signature",
...
def _reindex_payload(
    *,
    ...
    return {
        "from": {
            "channel_id": 1,
            "model_id": "memory_maintenance_lifecycle-reindex-model",
            "dimensions": 3,
            "signature": "memory_maintenance_lifecycle-reindex-signature",
...
async def test_corrupt_reindex_payload_does_not_rollback_generic_terminal_state(
    ...
    uid = "memory_maintenance_lifecycle-lifecycle-corrupt-reindex"
    source_collection = "memory_maintenance_lifecycle-corrupt-source"
    ...
    claimed = await claim_job(
        ...
        owner="memory_maintenance_lifecycle-corrupt-worker",
...
async def test_failed_reindex_finalization_marks_progress_and_store_failed(
    ...
    uid = "memory_maintenance_lifecycle-lifecycle-failed-reindex"
    source_collection = "memory_maintenance_lifecycle-failed-source"
    target_collection = "memory_maintenance_lifecycle-failed-target"
    index_revision = 6
    await configure_store(
        memory_session_factory,
        uid=uid,
    ...
    claimed = await claim_job(
        ...
        uid=uid,
        operation=LongTermMemoryMutationOperation.REINDEX,
        owner="memory_maintenance_lifecycle-failed-worker",
        payload=payload,
    )
    assert claimed is not None
    assert claimed.locked_by is not None
...
async def test_preparing_migration_pending_job_can_be_cancelled(
    ...
    uid = "memory_maintenance_lifecycle-lifecycle-cancel-preparing"
    source_collection = "memory_maintenance_lifecycle-cancel-source"
    target_collection = "memory_maintenance_lifecycle-cancel-target"
    ...
    job = await _prepare_embedding_migration(
        ...
        source=source,
        target=target,
        dedupe_key="memory_maintenance_lifecycle-cancel-preparing",
    )
    assert job.id is not None
...
async def test_switching_migration_rejects_cancellation(
    ...
    uid = "memory_maintenance_lifecycle-lifecycle-cancel-switching"
    source_collection = "memory_maintenance_lifecycle-switching-source"
    target_collection = "memory_maintenance_lifecycle-switching-target"
    source, target = _migration_configs(
        source_collection=source_collection,
        target_collection=target_collection,
    )
    ...
    job = await _prepare_embedding_migration(
        ...
        source=source,
        target=target,
        dedupe_key="memory_maintenance_lifecycle-cancel-switching",
    )
    assert job.id is not None
    claimed = await claim_job(
        memory_session_factory,
        ...
        operation=LongTermMemoryMutationOperation.EMBEDDING_MIGRATION,
        job_id=job.id,
        owner="memory_maintenance_lifecycle-switching-worker",
    )
    assert claimed is not None
...
async def test_expired_max_attempt_migration_is_failed_and_not_switched(
    ...
    uid = "memory_maintenance_lifecycle-lifecycle-expired-migration"
    source_collection = "memory_maintenance_lifecycle-expired-source"
    target_collection = "memory_maintenance_lifecycle-expired-target"
    source, target = _migration_configs(
        source_collection=source_collection,
        target_collection=target_collection,
    )
    ...
    job = await _prepare_embedding_migration(
        ...
        source=source,
        target=target,
        dedupe_key="memory_maintenance_lifecycle-expired-migration",
        max_attempts=1,
    ...
    claimed = await claim_job(
        ...
        operation=LongTermMemoryMutationOperation.EMBEDDING_MIGRATION,
        job_id=job.id,
        owner="memory_maintenance_lifecycle-expired-worker",
    )
    assert claimed is not None
...
async def test_expired_max_attempt_reindex_after_switch_fails_cleanup_only(
    ...
    uid = "memory_maintenance_lifecycle-lifecycle-expired-switched-reindex"
    source_collection = "memory_maintenance_lifecycle-expired-switched-reindex-source"
    target_collection = "memory_maintenance_lifecycle-expired-switched-reindex-target"
    ...
    job = await claim_job(
        ...
        uid=uid,
        operation=LongTermMemoryMutationOperation.REINDEX,
        owner="memory_maintenance_lifecycle-expired-switched-reindex-worker",
        payload=payload,
        max_attempts=1,
    )
    assert job is not None
...
async def test_expired_max_attempt_migration_after_switch_fails_cleanup_only(
    ...
    uid = "memory_maintenance_lifecycle-lifecycle-expired-switched-migration"
    source_collection = "memory_maintenance_lifecycle-expired-switched-migration-source"
    target_collection = "memory_maintenance_lifecycle-expired-switched-migration-target"
    source, target = _migration_configs(
        source_collection=source_collection,
        target_collection=target_collection,
    )
    ...
    job = await _prepare_embedding_migration(
        ...
        source=source,
        target=target,
        dedupe_key="memory_maintenance_lifecycle-expired-switched-migration",
        max_attempts=1,
    ...
    claimed = await claim_job(
        ...
        operation=LongTermMemoryMutationOperation.EMBEDDING_MIGRATION,
        job_id=job.id,
        owner="memory_maintenance_lifecycle-expired-switched-migration-worker",
    )
    assert claimed is not None
