...
async def test_migration_removal_delta_does_not_require_current_record(
    ...
    uid = f"memory_migration-removal-delta-{action.value}"
    collection_name = f"memory_migration-removal-target-{action.value}"
...
async def test_embedding_migration_switches_and_applies_snapshot_delta(
    ...
    uid = "memory_migration-migration-delta-user"
    old_collection = "memory_migration-migration-delta-old"
    target_collection = "memory_migration-migration-delta-target"
    source = {
        "channel_id": 1,
        "model_id": "memory-model-v1",
        "dimensions": 3,
        "signature": "memory_migration-source-signature",
    ...
    failed_target = {
        ...
        "dimensions": 4,
        "signature": "memory_migration-failed-signature",
        "collection": "memory_migration-migration-failed-target",
        "revision": 2,
    }
    target = {
        "channel_id": 9,
        ...
        "dimensions": 4,
        "signature": "memory_migration-target-signature",
    ...
    profile_a = Profile(
        uid=uid,
        name="memory_migration-migration-profile-a",
    ...
    profile_b = Profile(
        uid=uid,
        name="memory_migration-migration-profile-b",
        configs=ProfileConfig.model_validate(
            {
                "memory": {
                    "enabled": False,
    ...
    job = await _prepare_embedding_migration(
        memory_session_factory,
        uid=uid,
        dedupe_key="memory_migration-migration-delta",
        source=source,
        target=target,
    )
    assert job.id is not None
    ...
    claimed = await claim_job(
        ...
        operation=LongTermMemoryMutationOperation.EMBEDDING_MIGRATION,
        job_id=job.id,
        owner="memory_migration-migration-delta-worker",
    )
    ...
    result = await executor.execute_claimed(claimed, "memory_migration-migration-delta-worker")
...
async def test_embedding_migration_sample_query_failure_does_not_switch(
    ...
    uid = "memory_migration-migration-validation-user"
    old_collection = "memory_migration-migration-validation-old"
    target_collection = "memory_migration-migration-validation-target"
    source = {
        "channel_id": 3,
        "model_id": "memory-model-old",
        "dimensions": 3,
        "signature": "memory_migration-validation-source",
        "collection": old_collection,
        "revision": 1,
    }
    target = {
        ...
        "dimensions": 4,
        "signature": "memory_migration-validation-target",
        "collection": target_collection,
        "revision": 2,
    }
    await configure_store(
    ...
    job = await _prepare_embedding_migration(
        memory_session_factory,
        uid=uid,
        dedupe_key="memory_migration-migration-validation",
        source=source,
        target=target,
    )
    assert job.id is not None
    ...
    claimed = await claim_job(
        ...
        operation=LongTermMemoryMutationOperation.EMBEDDING_MIGRATION,
        job_id=job.id,
        owner="memory_migration-migration-validation-worker",
    )
    assert claimed is not None
    executor = MemoryJobExecutor(
        {LongTermMemoryMutationOperation.EMBEDDING_MIGRATION: handle_embedding_migration},
    ...
    with pytest.raises(MemoryJobRetryableError):
        await executor.execute_claimed(claimed, "memory_migration-migration-validation-worker")
...
async def test_embedding_migration_rejects_corrupted_non_sampled_target_vector(
    ...
    uid = "memory_migration-migration-vector-validation-user"
    old_collection = "memory_migration-migration-vector-validation-old"
    target_collection = "memory_migration-migration-vector-validation-target"
    source = {
        "channel_id": 5,
        "model_id": "memory-model-vector-source",
        "dimensions": 3,
        "signature": "memory_migration-vector-validation-source",
        "collection": old_collection,
        "revision": 1,
    }
    target = {
        ...
        "dimensions": 4,
        "signature": "memory_migration-vector-validation-target",
        "collection": target_collection,
        "revision": 2,
    }
    await configure_store(
    ...
    job = await _prepare_embedding_migration(
        memory_session_factory,
        uid=uid,
        dedupe_key="memory_migration-migration-vector-validation",
        source=source,
        target=target,
    )
    assert job.id is not None
    ...
    claimed = await claim_job(
        ...
        operation=LongTermMemoryMutationOperation.EMBEDDING_MIGRATION,
        job_id=job.id,
        owner="memory_migration-migration-vector-validation-worker",
    )
    assert claimed is not None
    executor = MemoryJobExecutor(
        {LongTermMemoryMutationOperation.EMBEDDING_MIGRATION: handle_embedding_migration},
    ...
    with pytest.raises(MemoryJobRetryableError):
        await executor.execute_claimed(claimed, "memory_migration-migration-vector-validation-worker")
...
async def test_embedding_migration_switch_is_fenced_after_expired_lease_recovery(
    ...
    uid = "memory_migration-migration-switch-fence-user"
    old_collection = "memory_migration-migration-switch-fence-old"
    target_collection = "memory_migration-migration-switch-fence-target"
    owner = "memory_migration-migration-switch-fence-worker"
    source = {
        "channel_id": 11,
        "model_id": "memory_migration-migration-switch-fence-source",
        "dimensions": 3,
        "signature": "memory_migration-migration-switch-fence-source-signature",
        "collection": old_collection,
        "revision": 1,
    }
    target = {
        "channel_id": 12,
        "model_id": "memory_migration-migration-switch-fence-target",
        "dimensions": 4,
        "signature": "memory_migration-migration-switch-fence-target-signature",
        "collection": target_collection,
        "revision": 2,
    }
    await configure_store(
    ...
    job = await _prepare_embedding_migration(
        memory_session_factory,
        uid=uid,
        dedupe_key="memory_migration-migration-switch-fence",
        source=source,
        target=target,
    )
    assert job.id is not None
    ...
    async def recover_expired_claim() -> None:
        ...
        async with memory_session_factory() as recovery_db:
            ...
            recovery = await memory_job_crud.recover_expired(
                recovery_db,
                max_attempts_error="memory_migration lease expired",
                commit=False,
            )
