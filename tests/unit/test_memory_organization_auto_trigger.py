...
async def memory_database(tmp_path: Path) -> AsyncIterator[async_sessionmaker[AsyncSession]]:
    database_path = (tmp_path / "memory-organization-auto-trigger-memory_organization_auto_trigger.db").as_posix()
...
def _chat_model(model_id: str = "organization-model", max_tokens: int = 20_000) -> dict[str, object]:
    return {
        ...
        "top_p": 0.8,
        "is_enabled": True,
        "description": "memory_organization_auto_trigger auto organization model",
        "advanced_settings": {"custom_headers": {"x-stage": "memory_organization_auto_trigger"}},
    }
