...
class MockSession:
    ...
    def post(self, *args, **kwargs):
        if self.raise_error and not isinstance(
            self.raise_error, aiohttp.ClientConnectorError
        ):
            raise self.raise_error
        return self.response

    async def __aenter__(self):
        if self.raise_error and isinstance(
            self.raise_error, aiohttp.ClientConnectorError
        ):
            raise self.raise_error
        return self
...
async def test_generate_success():
    ...
    with patch(
        ...
        side_effect=get_mock_session_factory(response=mock_response),
    ):
...
async def test_generate_connection_error():
    ...
    conn_error = ClientConnectorError(
        connection_key=mock_key, os_error=OSError("DNS error")
    )

    with patch(
        "aiohttp.ClientSession",
        side_effect=get_mock_session_factory(raise_error=conn_error),
    ):
        with pytest.raises(LLMException) as excinfo:
            await LLMClient.generate(
                api_key="sk-test",
