...
async def session_factory(tmp_path: Path) -> AsyncIterator[async_sessionmaker[AsyncSession]]:
    database_path = tmp_path / "knowledge-organization-stage15.db"
...
async def _create_managed_container(db: AsyncSession, *, suffix: str = "main") -> KnowledgeBase:
    channel = ModelChannel(
        name=f"organization-stage15-channel-{suffix}",
        api_key="test-key",
    ...
    prompt = PromptLibrary(uid="user-1", name=f"organization-stage15-prompt-{suffix}", content="prompt")
    db.add(prompt)
    await db.flush()

    profile = Profile(
        uid="user-1",
        name=f"organization-stage15-profile-{suffix}",
        prompt_id=prompt.id,
        configs={},
    )
    db.add(profile)
    ...
    knowledge_base = KnowledgeBase(
        uid="user-1",
        name=f"managed-stage15-{suffix}",
        embedding_channel_id=channel.id,
        embedding_model_id="embedding-model",
        embedding_dimensions=1536,
        collection_name=f"managed-stage15-{suffix}-collection",
        ...
        active_collection_name=f"managed-stage15-{suffix}-active",
        index_revision=5,
    )
...
async def _create_running_organization_job(
    ...
    *,
    ...
    owner: str = "stage15-parent-worker",
) -> KnowledgeJob:
    ...
    job, created = await knowledge_job_crud.create(
        ...
        uid="user-1",
        operation=operation,
        dedupe_key=f"stage15-parent:{knowledge_base_id}:{suffix}",
...
async def _create_pending_organization_job(
    ...
    *,
    ...
    job, created = await knowledge_job_crud.create(
        ...
        dedupe_key=f"stage15-pending:{knowledge_base_id}:{suffix}",
        request_hash=hashlib.sha256(suffix.encode("utf-8")).hexdigest(),
        active_change_key=f"kb-organization:{knowledge_base_id}",
        status=KnowledgeJobStatus.PENDING,
        knowledge_base_id=knowledge_base_id,
...
async def _complete_stage(
    ...
    *,
    ...
    work_key = hashlib.sha256(f"stage15:{snapshot.id}:{stage_key}".encode()).hexdigest()
    model_key = hashlib.sha256(b"stage15-model").hexdigest()
...
@pytest.mark.asyncio
async def test_stage15_publication_validates_complete_plan_and_creates_mutation_children(session_factory):
    owner = "stage15-publication-owner"
...
async def test_stage15_publication_rejects_changed_source_state_without_children(session_factory, change: str):
    owner = f"stage15-stale-{change}"
    async with session_factory() as db:
...
async def test_stage15_failed_or_cancelled_parent_cancels_pending_children_and_releases_locks(
    session_factory,
    terminal: KnowledgeJobStatus,
):
    owner = f"stage15-terminal-{terminal.value}"
    async with session_factory() as db:
        ...
        child_ids = [
            (
                await knowledge_job_crud.create(
                    ...
                    dedupe_key=f"stage15-child:{parent.id}:{index}",
                    request_hash=hashlib.sha256(f"child-{index}".encode()).hexdigest(),
                    active_change_key=f"stage15-child-active:{parent.id}:{index}",
                    status=KnowledgeJobStatus.PENDING,
                    knowledge_base_id=knowledge_base.id,
                    knowledge_id=item.id,
                    expected_version=1,
...
@pytest.mark.asyncio
async def test_stage15_successful_parent_waits_for_all_children_before_releasing_locks(session_factory):
    owner = "stage15-success-parent"
    async with session_factory() as db:
        ...
        for index, item in enumerate(items):
            child, created = await knowledge_job_crud.create(
                ...
                parent_job_id=parent.id,
                operation=KnowledgeJobOperation.ORGANIZE_MUTATION,
                dedupe_key=f"stage15-success-child:{parent.id}:{index}",
                request_hash=hashlib.sha256(f"success-child-{index}".encode()).hexdigest(),
                active_change_key=f"stage15-success-child-active:{parent.id}:{index}",
                status=KnowledgeJobStatus.PENDING,
                knowledge_base_id=knowledge_base.id,
                knowledge_id=item.id,
                expected_version=1,
    ...
    for index in range(2):
        async with session_factory() as db:
            ...
            child_owner = f"stage15-child-worker-{index}"
            claimed = await knowledge_job_crud.try_claim(
                db,
                uid="user-1",
                job_id=child.id,
...
@pytest.mark.asyncio
async def test_stage15_organization_parent_cannot_succeed_with_failed_child(session_factory):
    owner = "stage15-failed-child-parent"
    async with session_factory() as db:
        ...
        child = KnowledgeJob(
            ...
            parent_job_id=parent.id,
            operation=KnowledgeJobOperation.ORGANIZE_MUTATION,
            dedupe_key="stage15-failed-child-parent-child",
            request_hash="f" * 64,
            status=KnowledgeJobStatus.FAILED,
            knowledge_base_id=knowledge_base.id,
...
@pytest.mark.asyncio
async def test_stage15_consumer_waits_for_failed_organization_child_before_failing_parent(session_factory):
    owner = "stage15-consumer-parent-worker"
    child_owner = "stage15-consumer-child-worker"
    async with session_factory() as db:
        ...
        child, created = await knowledge_job_crud.create(
            ...
            parent_job_id=parent.id,
            operation=KnowledgeJobOperation.ORGANIZE_MUTATION,
            dedupe_key=f"stage15-consumer-child:{parent.id}",
            request_hash=hashlib.sha256(b"stage15-consumer-child").hexdigest(),
            active_change_key=f"stage15-consumer-child-active:{parent.id}",
            status=KnowledgeJobStatus.PENDING,
            knowledge_base_id=knowledge_base.id,
            knowledge_id=1,
            expected_version=1,
...
async def test_stage15_auto_organization_obeys_threshold_without_interval(
    session_factory,
...
@pytest.mark.asyncio
async def test_stage15_repeated_submission_is_idempotent_and_busy_auto_submission_does_not_duplicate(session_factory):
    async with session_factory() as db:
        ...
        first_deduped = await submit_knowledge_organization(
            ...
            source="auto",
            dedupe_key="stage15-same-request",
        )
...
@pytest.mark.asyncio
async def test_stage15_job_detail_and_list_expose_snapshot_progress_and_zero_missing_snapshot_progress(session_factory):
    async with session_factory() as db:
        ...
        parent.payload = {"source": "stage15", "snapshot_id": snapshot.id}
        await db.commit()
...
@pytest.mark.asyncio
async def test_stage15_job_view_counts_only_fully_published_organization_mutations(session_factory):
    async with session_factory() as db:
        ...
        mutation = KnowledgeJob(
            ...
            parent_job_id=parent.id,
            operation=KnowledgeJobOperation.ORGANIZE_MUTATION,
            dedupe_key="stage15-publication-count-mutation",
            request_hash="a" * 64,
            status=KnowledgeJobStatus.SUCCEEDED,
            knowledge_base_id=knowledge_base.id,
            result={},
        ...
        publication = KnowledgeJob(
            ...
            dedupe_key="stage15-publication-count-update",
            request_hash="b" * 64,
            status=KnowledgeJobStatus.PENDING,
            knowledge_base_id=knowledge_base.id,
        )
        cleanup = KnowledgeJob(
            ...
            dedupe_key="stage15-publication-count-delete",
            request_hash="e" * 64,
            status=KnowledgeJobStatus.SUCCEEDED,
            knowledge_base_id=knowledge_base.id,
        )
...
@pytest.mark.asyncio
async def test_stage15_parent_cancellation_rolls_back_committed_unpublished_mutation(session_factory):
    parent_owner = "stage15-cancel-rollback-parent"
    mutation_owner = "stage15-cancel-rollback-mutation"
...
@pytest.mark.asyncio
async def test_stage15_parent_cancellation_rolls_back_committed_unpublished_merge_mutation(session_factory):
    parent_owner = "stage15-cancel-merge-parent"
    mutation_owner = "stage15-cancel-merge-mutation"
...
async def test_stage15_merge_releases_secondary_unique_fields_before_primary_update(
    session_factory,
    collision_field: str,
):
    parent_owner = f"stage15-merge-unique-{collision_field}-parent"
    mutation_owner = f"stage15-merge-unique-{collision_field}-mutation"
...
@pytest.mark.asyncio
async def test_stage15_parent_cancellation_before_mutation_submission_is_fenced(session_factory, monkeypatch):
    parent_owner = "stage15-cancel-fence-parent"
    mutation_owner = "stage15-cancel-fence-mutation"
...
@pytest.mark.asyncio
async def test_stage15_publication_cancel_barrier_rejects_plan_without_mutation_children(
    session_factory,
    monkeypatch,
):
    owner = "stage15-publication-cancel-barrier"
...
@pytest.mark.asyncio
async def test_stage15_publication_uses_bounded_snapshot_and_current_item_pages(session_factory, monkeypatch):
    import app.core.knowledge.organization_publication as publication_module

    owner = "stage15-publication-bounded-pages"
...
@pytest.mark.asyncio
async def test_stage15_publication_uses_default_pages_for_401_item_keep_plan(session_factory, monkeypatch):
    import app.core.knowledge.organization_publication as publication_module

    owner = "stage15-publication-default-large-pages"
...
@pytest.mark.asyncio
async def test_stage15_snapshot_cancel_barrier_rejects_snapshot_without_lock(
    session_factory,
    monkeypatch,
):
    owner = "stage15-snapshot-cancel-barrier"
...
@pytest.mark.asyncio
async def test_stage15_auto_organization_counts_candidates_without_loading_full_list(session_factory, monkeypatch):
...
@pytest.mark.asyncio
async def test_stage15_waiting_organization_parent_does_not_block_single_worker_child_execution(
    session_factory,
):
    async with session_factory() as db:
        ...
        parent, created = await knowledge_job_crud.create(
            ...
            uid="user-1",
            operation=KnowledgeJobOperation.MANUAL_ORGANIZE,
            dedupe_key="stage15-single-worker-parent",
            request_hash="c" * 64,
        ...
        async with session_factory() as db:
            ...
            for index in range(3):
                unrelated, created = await knowledge_job_crud.create(
                    ...
                    dedupe_key=f"stage15-single-worker-unrelated:{context.job.id}:{index}",
            ...
            child, created = await knowledge_job_crud.create(
                ...
                uid=context.job.uid,
                operation=KnowledgeJobOperation.ORGANIZE_MUTATION,
                dedupe_key=f"stage15-single-worker-child:{context.job.id}",
                request_hash="d" * 64,
...
@pytest.mark.asyncio
async def test_stage15_real_worker_publishes_update_and_completes_organization_parent(
    session_factory,
    monkeypatch,
):
    import app.core.knowledge.organization_run as organization_run

    parent_owner = "stage15-real-worker-parent"
    mutation_owner = "stage15-real-worker-mutation"
    update_owner = "stage15-real-worker-update"
    ...
    assert collection_name == "managed-stage15-real-worker-active"
...
@pytest.mark.asyncio
async def test_stage15_real_worker_publishes_merge_with_secondary_identity_and_completes_organization_parent(
    session_factory,
    monkeypatch,
):
    import app.core.knowledge.organization_run as organization_run

    parent_owner = "stage15-real-worker-merge-parent"
    mutation_owner = "stage15-real-worker-merge-mutation"
    delete_owner = "stage15-real-worker-merge-delete"
    update_owner = "stage15-real-worker-merge-update"
...
@pytest.mark.asyncio
async def test_stage15_real_worker_failed_update_fails_parent_and_rolls_back_item(
    session_factory,
    monkeypatch,
):
    import app.core.knowledge.organization_run as organization_run

    parent_owner = "stage15-real-worker-failure-parent"
    mutation_owner = "stage15-real-worker-failure-mutation"
    update_owner = "stage15-real-worker-failure-update"
