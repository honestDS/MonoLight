...
async def managed_knowledge_tool_database(tmp_path) -> AsyncIterator[async_sessionmaker[AsyncSession]]:
    database_path = tmp_path / "managed-knowledge-tool-managed_knowledge_tool.db"
...
async def _create_runtime(
    ...
    async with session_factory() as db:
        channel = ModelChannel(
            name="managed_knowledge_tool-embedding",
        ...
        profile = Profile(
            name="Managed Knowledge Profile",
            uid="user-1",
...
def _executor(
    ...
    *,
    ...
    context = build_dispatch_context(
        ...
        uid="user-1",
        session_id="session-managed_knowledge_tool",
        profile=profile,
        db=db,
...
@pytest.mark.asyncio
async def test_managed_knowledge_tool_first_knowledge_create_lazily_creates_managed_container_and_safe_job(
    managed_knowledge_tool_database: async_sessionmaker[AsyncSession],
) -> None:
    profile, channel = await _create_runtime(managed_knowledge_tool_database)
    knowledge_content = "The billing service uses idempotency keys for payment retries."

    async with managed_knowledge_tool_database() as db:
    ...
    async with managed_knowledge_tool_database() as db:
        ...
        assert job.source_session_id == "session-managed_knowledge_tool"
...
@pytest.mark.asyncio
async def test_managed_knowledge_tool_managed_update_and_delete_use_current_profile_container(
    managed_knowledge_tool_database: async_sessionmaker[AsyncSession],
) -> None:
    profile, _channel = await _create_runtime(managed_knowledge_tool_database)
...
@pytest.mark.asyncio
async def test_managed_knowledge_tool_managed_version_conflict_is_rejected_without_mutation(
    managed_knowledge_tool_database: async_sessionmaker[AsyncSession],
) -> None:
    profile, _channel = await _create_runtime(managed_knowledge_tool_database)
...
@pytest.mark.asyncio
async def test_managed_knowledge_tool_user_manual_edit_can_lock_item_against_llm_update_and_delete(
    managed_knowledge_tool_database: async_sessionmaker[AsyncSession],
) -> None:
    profile, _channel = await _create_runtime(managed_knowledge_tool_database)
...
@pytest.mark.asyncio
async def test_managed_knowledge_tool_user_knowledge_base_document_cannot_be_managed_mutation_target(
    managed_knowledge_tool_database: async_sessionmaker[AsyncSession],
) -> None:
    profile, channel = await _create_runtime(managed_knowledge_tool_database)
...
@pytest.mark.asyncio
async def test_managed_knowledge_tool_user_can_reenable_llm_maintenance_for_current_version(
    managed_knowledge_tool_database: async_sessionmaker[AsyncSession],
) -> None:
    profile, _channel = await _create_runtime(managed_knowledge_tool_database)
...
@pytest.mark.asyncio
async def test_managed_knowledge_tool_profile_delete_lock_blocks_managed_update_until_profile_is_gone(
    managed_knowledge_tool_database: async_sessionmaker[AsyncSession],
) -> None:
    profile, _channel = await _create_runtime(managed_knowledge_tool_database)
    ...
    async with managed_knowledge_tool_database() as delete_db:
        ...
        async def update_after_delete() -> dict:
            async with managed_knowledge_tool_database() as update_db:
                executor = _executor(
                    update_db,
                    profile,
...
@pytest.mark.asyncio
async def test_managed_knowledge_tool_duplicate_create_does_not_expose_existing_write_identity(
    managed_knowledge_tool_database: async_sessionmaker[AsyncSession],
) -> None:
    profile, _channel = await _create_runtime(managed_knowledge_tool_database)
...
@pytest.mark.asyncio
async def test_managed_knowledge_tool_failed_job_replay_reports_failed_instead_of_accepted(
    managed_knowledge_tool_database: async_sessionmaker[AsyncSession],
) -> None:
    profile, _channel = await _create_runtime(managed_knowledge_tool_database)
...
@pytest.mark.asyncio
async def test_managed_knowledge_tool_managed_update_cannot_cross_profile_boundary(
    managed_knowledge_tool_database: async_sessionmaker[AsyncSession],
) -> None:
    profile_a, _channel = await _create_runtime(managed_knowledge_tool_database)

    async with managed_knowledge_tool_database() as db:
        profile_b = Profile(
            name="Managed Knowledge Other Profile",
            uid="user-1",
            configs={"memory": {"enabled": True}},
        )
        db.add(profile_b)
...
@pytest.mark.asyncio
async def test_managed_knowledge_tool_managed_knowledge_base_delete_cascades_and_recreates_new_container(
    managed_knowledge_tool_database: async_sessionmaker[AsyncSession],
) -> None:
    profile, _channel = await _create_runtime(managed_knowledge_tool_database)
...
@pytest.mark.asyncio
async def test_managed_knowledge_tool_managed_knowledge_base_delete_logs_active_organization_lock_and_still_deletes(
    managed_knowledge_tool_database: async_sessionmaker[AsyncSession],
...
@pytest.mark.asyncio
async def test_managed_knowledge_tool_profile_delete_logs_active_organization_lock_and_still_deletes(
    managed_knowledge_tool_database: async_sessionmaker[AsyncSession],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    profile, _channel = await _create_runtime(managed_knowledge_tool_database)
...
@pytest.mark.asyncio
async def test_managed_knowledge_tool_profile_create_retries_once_after_container_conflict(
    managed_knowledge_tool_database: async_sessionmaker[AsyncSession],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    profile, _channel = await _create_runtime(managed_knowledge_tool_database)
    ...
    async with managed_knowledge_tool_database() as db:
        result = await knowledge_job_manager_module.knowledge_job_manager.submit_create_for_profile(
            ...
            dedupe_key="managed_knowledge_tool-container-retry",
            source_session_id="session-managed_knowledge_tool",
