...
async def memory_database(tmp_path) -> AsyncIterator[async_sessionmaker[AsyncSession]]:
    database_path = tmp_path / "memory-capacity-reservation-memory_capacity_reservation.db"
...
async def _create_record(
    ...
    *,
    ...
    values = {
        ...
        "source_id": "memory_capacity_reservation-capacity-seed",
        "change_evidence": "memory_capacity_reservation capacity seed",
        "is_active": is_active,
        "pinned": pinned,
...
async def _submit_create(
    ...
    *,
    ...
    source_id: str = "memory_capacity_reservation-capacity-request",
...
async def test_full_capacity_create_accepts_replacement_and_reserves_candidate_with_strict_payload(
    ...
    assert job.payload == {
        "publication": {
            ...
            "source_id": "memory_capacity_reservation-capacity-request",
    ...
    async with memory_database() as db:
        with pytest.raises(MemoryConflictError) as exc_info:
            await memory_service.create(
                ...
                memory_type=LongTermMemoryType.FACT,
                source=LongTermMemorySource.USER_API,
                source_id="memory_capacity_reservation-suppressed-active-capacity",
            )
    ...
    async with memory_database() as db:
        ...
        with pytest.raises(MemoryConflictError) as exc_info:
            await memory_service.update(
                ...
                memory_type=LongTermMemoryType.FACT,
                source=LongTermMemorySource.USER_API,
                source_id="memory_capacity_reservation-capacity-update-request",
            )
    ...
    async def submit_racing_create(dedupe_key: str, memory_key: str):
        async with memory_database() as db:
            ...
            return await memory_service.create(
                ...
                memory_type=LongTermMemoryType.FACT,
                source=LongTermMemorySource.USER_API,
                source_id="memory_capacity_reservation-race-request",
            )
    ...
    async def submit_duplicate():
        async with memory_database() as db:
            ...
            return await memory_service.create(
                ...
                memory_type=LongTermMemoryType.FACT,
                source=LongTermMemorySource.USER_API,
                source_id="memory_capacity_reservation-dedupe-request",
            )
...
async def test_over_limit_legacy_data_rejects_growth_but_allows_safe_mutations_and_shrinking(
    ...
    async with memory_database() as db:
        with pytest.raises(MemoryConflictError) as create_error:
            await memory_service.create(
                ...
                memory_type=LongTermMemoryType.FACT,
                source=LongTermMemorySource.USER_API,
                source_id="memory_capacity_reservation-over-limit-create",
            )
        with pytest.raises(MemoryConflictError) as update_error:
            await memory_service.update(
                ...
                memory_type=LongTermMemoryType.FACT,
                source=LongTermMemorySource.USER_API,
                source_id="memory_capacity_reservation-over-limit-update",
            )
    ...
    async with memory_database() as db:
        shortened = await memory_service.update(
            ...
            memory_type=LongTermMemoryType.FACT,
            source=LongTermMemorySource.USER_API,
            source_id="memory_capacity_reservation-over-limit-shortening",
        )
    ...
    async with memory_database() as db:
        deleted = await memory_service.delete(
            ...
            expected_version=1,
            source=LongTermMemorySource.USER_API,
            source_id="memory_capacity_reservation-over-limit-delete",
        )
...
async def test_oversized_active_record_blocks_create_but_allows_strictly_shorter_update(
    ...
    async with memory_database() as db:
        with pytest.raises(MemoryConflictError) as create_error:
            await memory_service.create(
                ...
                memory_type=LongTermMemoryType.FACT,
                source=LongTermMemorySource.USER_API,
                source_id="memory_capacity_reservation-content-over-limit-create",
            )
    ...
    async with memory_database() as db:
        shortened = await memory_service.update(
            ...
            memory_type=LongTermMemoryType.FACT,
            source=LongTermMemorySource.USER_API,
            source_id="memory_capacity_reservation-content-over-limit-update",
        )
