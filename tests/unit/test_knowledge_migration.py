...
import app.core.knowledge.embedding_migration as embedding_migration_module
...
async def migration_database(tmp_path) -> AsyncIterator[async_sessionmaker[AsyncSession]]:
    database_path = tmp_path / "knowledge-migration-knowledge_migration.db"
...
async def test_migration_rebuilds_user_documents_and_switches_only_after_validation(
    ...
    async with migration_database() as db:
        job = await prepare_knowledge_base_embedding_migration(
            ...
            target_collection_name="knowledge_migration-user-target",
            dedupe_key="knowledge_migration-user-migration",
        )
    ...
    async with migration_database() as db:
        ...
        assert current.target_collection_name == "knowledge_migration-user-target"
    ...
    claimed = await _claim(
        ...
        uid=kb.uid,
        job_id=job.id,
        owner="knowledge_migration-worker",
    )
    executor = create_default_knowledge_job_executor(session_factory=migration_database)
    execution = await executor.execute_claimed(claimed, "knowledge_migration-worker")
    ...
    assert current is not None
    assert current.active_collection_name == "knowledge_migration-user-target"
    ...
    target_items = backend.collections["knowledge_migration-user-target"]["items"]
...
async def test_migration_applies_monotonic_delta_for_new_document_after_snapshot(
    ...
    async with migration_database() as db:
        job = await prepare_knowledge_base_embedding_migration(
            ...
            target_dimensions=3,
            target_signature="signature-v2",
            target_collection_name="knowledge_migration-delta-target",
            dedupe_key="knowledge_migration-delta-migration",
        )
    assert job.id is not None
    claimed = await _claim(
        migration_database,
        uid=kb.uid,
        job_id=job.id,
        owner="knowledge_migration-delta-worker",
    )
    context = KnowledgeJobExecutionContext(
        job=claimed,
        worker_id="knowledge_migration-delta-worker",
    ...
    assert migrated_document is not None
    assert set(migrated_document.chunk_ids).issubset(set(backend.collections["knowledge_migration-delta-target"]["items"]))
...
async def test_managed_knowledge_mutation_automatically_records_migration_delta(
    ...
    async with migration_database() as db:
        job = await prepare_knowledge_base_embedding_migration(
            ...
            target_dimensions=3,
            target_signature="signature-v2",
            target_collection_name="knowledge_migration-managed-target",
            dedupe_key="knowledge_migration-managed-migration",
        )
...
async def test_managed_update_and_delete_record_monotonic_migration_deltas(
    ...
    async with migration_database() as db:
        job = await prepare_knowledge_base_embedding_migration(
            ...
            target_dimensions=3,
            target_signature="signature-v2",
            target_collection_name="knowledge_migration-managed-change-target",
            dedupe_key="knowledge_migration-managed-change-migration",
        )
...
async def test_delta_arriving_during_validation_returns_to_catchup(
    ...
    async with migration_database() as db:
        job = await prepare_knowledge_base_embedding_migration(
            ...
            target_dimensions=3,
            target_signature="signature-v2",
            target_collection_name="knowledge_migration-validation-delta-target",
            dedupe_key="knowledge_migration-validation-delta-migration",
        )
    assert job.id is not None
    claimed = await _claim(
        migration_database,
        uid=kb.uid,
        job_id=job.id,
        owner="knowledge_migration-validation-delta-worker",
    )
    context = KnowledgeJobExecutionContext(
        job=claimed,
        worker_id="knowledge_migration-validation-delta-worker",
        session_factory=migration_database,
    )
    ...
    assert current.migration_delta_applied_watermark == 1
    assert migrated_document is not None
    assert set(migrated_document.chunk_ids).issubset(set(backend.collections["knowledge_migration-validation-delta-target"]["items"]))
...
async def test_document_delete_after_snapshot_is_removed_from_target(
    ...
    async with migration_database() as db:
        ...
        job = await prepare_knowledge_base_embedding_migration(
            ...
            target_dimensions=3,
            target_signature="signature-v2",
            target_collection_name="knowledge_migration-delete-target",
            dedupe_key="knowledge_migration-delete-migration",
        )
    assert job.id is not None
    claimed = await _claim(
        migration_database,
        uid=kb.uid,
        job_id=job.id,
        owner="knowledge_migration-delete-worker",
    )
    context = KnowledgeJobExecutionContext(
        job=claimed,
        worker_id="knowledge_migration-delete-worker",
        session_factory=migration_database,
    )
    payload = await migration_prepare_module._prepare_migration(context, claimed.payload)
    await migration_build_module._build_migration(context, payload)
    assert backend.collections["knowledge_migration-delete-target"]["items"]
...
async def test_switch_transaction_failure_keeps_old_collection_active(
    ...
    async with migration_database() as db:
        ...
        job = await prepare_knowledge_base_embedding_migration(
            ...
            target_dimensions=3,
            target_signature="signature-v2",
            target_collection_name="knowledge_migration-switch-failure-target",
            dedupe_key="knowledge_migration-switch-failure-migration",
        )
    assert job.id is not None
    claimed = await _claim(
        migration_database,
        uid=kb.uid,
        job_id=job.id,
        owner="knowledge_migration-switch-failure-worker",
    )
    context = KnowledgeJobExecutionContext(
        job=claimed,
        worker_id="knowledge_migration-switch-failure-worker",
        session_factory=migration_database,
    )
    payload = await migration_prepare_module._prepare_migration(context, claimed.payload)
    await migration_build_module._build_migration(context, payload)
    ...
    assert current.migration_status == KnowledgeBaseMigrationStatus.VALIDATING
    assert current.target_collection_name == "knowledge_migration-switch-failure-target"
...
async def test_switch_reuses_validated_plan_without_reloading_sources_under_lock(
    ...
    async with migration_database() as db:
        ...
        job = await prepare_knowledge_base_embedding_migration(
            ...
            target_dimensions=3,
            target_signature="signature-v2",
            target_collection_name="knowledge_migration-short-switch-target",
            dedupe_key="knowledge_migration-short-switch-migration",
        )
    assert job.id is not None
    claimed = await _claim(
        migration_database,
        uid=kb.uid,
        job_id=job.id,
        owner="knowledge_migration-short-switch-worker",
    )
    context = KnowledgeJobExecutionContext(
        job=claimed,
        worker_id="knowledge_migration-short-switch-worker",
        session_factory=migration_database,
    )
    payload = await migration_prepare_module._prepare_migration(context, claimed.payload)
    await migration_build_module._build_migration(context, payload)
    ...
    async with migration_database() as db:
        ...
        current_document = await db.get(KnowledgeBaseDocument, document.id)
    assert current is not None
    assert current.active_collection_name == "knowledge_migration-short-switch-target"
    assert current.migration_status == KnowledgeBaseMigrationStatus.SUCCEEDED
    assert current_document is not None
...
async def test_switch_batches_managed_vector_reference_updates(
    ...
    async with migration_database() as db:
        ...
        job = await prepare_knowledge_base_embedding_migration(
            ...
            target_dimensions=3,
            target_signature="signature-v2",
            target_collection_name="knowledge_migration-managed-switch-target",
            dedupe_key="knowledge_migration-managed-switch-migration",
        )
    assert job.id is not None
    claimed = await _claim(
        migration_database,
        uid=kb.uid,
        job_id=job.id,
        owner="knowledge_migration-managed-switch-worker",
    )
    context = KnowledgeJobExecutionContext(
        job=claimed,
        worker_id="knowledge_migration-managed-switch-worker",
        session_factory=migration_database,
    )
    execution = await migration_module.handle_embedding_migration(context)
    assert execution.finalized is True
    ...
    assert current is not None
    assert current.active_collection_name == "knowledge_migration-managed-switch-target"
    assert current.active_embedding_model_id == "embedding-v2"
    assert current.active_embedding_dimensions == 3
    ...
    target_ids = set(backend.collections["knowledge_migration-managed-switch-target"]["items"])
...
async def test_switch_rejects_validated_plan_when_delta_arrives_before_lock(
    ...
    async with migration_database() as db:
        job = await prepare_knowledge_base_embedding_migration(
            ...
            target_dimensions=3,
            target_signature="signature-v2",
            target_collection_name="knowledge_migration-switch-fence-target",
            dedupe_key="knowledge_migration-switch-fence-migration",
        )
    assert job.id is not None
    claimed = await _claim(
        migration_database,
        uid=kb.uid,
        job_id=job.id,
        owner="knowledge_migration-switch-fence-worker",
    )
    context = KnowledgeJobExecutionContext(
        job=claimed,
        worker_id="knowledge_migration-switch-fence-worker",
        session_factory=migration_database,
    )
    payload = await migration_prepare_module._prepare_migration(context, claimed.payload)
    await migration_build_module._build_migration(context, payload)
...
async def test_validation_failure_never_switches_active_collection(
    ...
    async with migration_database() as db:
        ...
        job = await prepare_knowledge_base_embedding_migration(
            ...
            target_dimensions=3,
            target_signature="signature-v2",
            target_collection_name="knowledge_migration-validation-target",
            dedupe_key="knowledge_migration-validation-migration",
        )
    assert job.id is not None
    claimed = await _claim(
        migration_database,
        uid=kb.uid,
        job_id=job.id,
        owner="knowledge_migration-validation-worker",
    )
    ...
    context = KnowledgeJobExecutionContext(
        job=claimed,
        worker_id="knowledge_migration-validation-worker",
        session_factory=migration_database,
    )
    with pytest.raises(KnowledgeJobRetryableError):
    ...
    assert current is not None
    assert current.active_collection_name == kb.active_collection_name
    assert current.target_collection_name == "knowledge_migration-validation-target"
    assert current.migration_status == KnowledgeBaseMigrationStatus.VALIDATING
...
async def test_old_collection_cleanup_failure_is_independent_from_migration_success(
    ...
    async with migration_database() as db:
        job = await prepare_knowledge_base_embedding_migration(
            ...
            target_dimensions=3,
            target_signature="signature-v2",
            target_collection_name="knowledge_migration-cleanup-target",
            dedupe_key="knowledge_migration-cleanup-migration",
        )
    assert job.id is not None
    claimed = await _claim(
        migration_database,
        uid=kb.uid,
        job_id=job.id,
        owner="knowledge_migration-cleanup-migration-worker",
    )
    executor = create_default_knowledge_job_executor(session_factory=migration_database)
    migration_result = await executor.execute_claimed(
        claimed,
        "knowledge_migration-cleanup-migration-worker",
    )
    ...
    cleanup_claim = await _claim(
        ...
        owner="knowledge_migration-cleanup-worker",
    )
    cleanup_context = KnowledgeJobExecutionContext(
        job=cleanup_claim,
        worker_id="knowledge_migration-cleanup-worker",
        session_factory=migration_database,
    )
    with pytest.raises(KnowledgeJobRetryableError):
    ...
    async with migration_database() as db:
        current = await db.get(KnowledgeBase, kb.id)
    assert current is not None
    assert current.active_collection_name == "knowledge_migration-cleanup-target"
    assert current.migration_status == KnowledgeBaseMigrationStatus.SUCCEEDED
    ...
    async with migration_database() as db:
        with pytest.raises(KnowledgeJobTargetBusyError):
            await prepare_knowledge_base_embedding_migration(
                ...
                target_collection_name="knowledge_migration-cleanup-next-target",
                dedupe_key="knowledge_migration-cleanup-next-migration",
            )
        await db.rollback()
    ...
    cleanup_retry_claim = await _claim(
        ...
        uid=kb.uid,
        job_id=cleanup_job.id,
        owner="knowledge_migration-cleanup-retry-worker",
    )
    cleanup_retry_context = KnowledgeJobExecutionContext(
        job=cleanup_retry_claim,
        worker_id="knowledge_migration-cleanup-retry-worker",
        session_factory=migration_database,
    )
...
async def test_worker_lease_expiry_resumes_migration_even_at_attempt_limit(
    ...
    async with migration_database() as db:
        job = await prepare_knowledge_base_embedding_migration(
            ...
            target_dimensions=3,
            target_signature="signature-v2",
            target_collection_name="knowledge_migration-restart-target",
            dedupe_key="knowledge_migration-restart-migration",
            max_attempts=1,
        )
    assert job.id is not None
    claimed = await _claim(
        ...
        uid=kb.uid,
        job_id=job.id,
        owner="knowledge_migration-restart-worker",
    )
    context = KnowledgeJobExecutionContext(
        job=claimed,
        worker_id="knowledge_migration-restart-worker",
        session_factory=migration_database,
    )
    await migration_prepare_module._prepare_migration(context, claimed.payload)
    ...
    assert current_kb.target_collection_name == "knowledge_migration-restart-target"
    reclaimed = await _claim(
        migration_database,
        uid=kb.uid,
        job_id=job.id,
        owner="knowledge_migration-restart-worker-2",
    )
...
async def test_cancelled_migration_keeps_old_collection_and_cleans_target(
    ...
    async with migration_database() as db:
        job = await prepare_knowledge_base_embedding_migration(
            ...
            target_dimensions=3,
            target_signature="signature-v2",
            target_collection_name="knowledge_migration-cancel-target",
            dedupe_key="knowledge_migration-cancel-migration",
        )
    assert job.id is not None
    claimed = await _claim(
        migration_database,
        uid=kb.uid,
        job_id=job.id,
        owner="knowledge_migration-cancel-worker",
    )
    context = KnowledgeJobExecutionContext(
        job=claimed,
        worker_id="knowledge_migration-cancel-worker",
        session_factory=migration_database,
    )
    await migration_prepare_module._prepare_migration(context, claimed.payload)
    assert "knowledge_migration-cancel-target" in backend.collections
    ...
    cleanup_claim = await _claim(
        ...
        uid=kb.uid,
        job_id=cleanup_job.id,
        owner="knowledge_migration-cancel-cleanup-worker",
    )
    cleanup_context = KnowledgeJobExecutionContext(
        job=cleanup_claim,
        worker_id="knowledge_migration-cancel-cleanup-worker",
        session_factory=migration_database,
    )
    cleanup_result = await migration_module.handle_migration_target_cleanup(cleanup_context)
    assert cleanup_result.result["collection"] == "knowledge_migration-cancel-target"
    assert "knowledge_migration-cancel-target" not in backend.collections


@pytest.mark.asyncio
async def test_user_migration_submission_converts_embedding_http_error_to_business_error(
    migration_database: async_sessionmaker[AsyncSession],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    knowledge_base = await _create_container(migration_database)
    assert knowledge_base.id is not None

    async def failed_load_runtime_config(*_args, **_kwargs):
        raise HTTPException(status_code=404, detail=ERR_PROFILE_EMBEDDING_CHANNEL_NOT_FOUND)

    monkeypatch.setattr(
        embedding_migration_module,
        "load_embedding_runtime_config",
        failed_load_runtime_config,
    )

    async with migration_database() as db:
        with pytest.raises(ParameterException) as exc_info:
            await embedding_migration_module.submit_user_knowledge_base_embedding_migration(
                db,
...
@pytest.mark.asyncio
async def test_user_migration_probe_failure_keeps_target_and_job_unset(
    migration_database: async_sessionmaker[AsyncSession],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    knowledge_base = await _create_container(migration_database)
    assert knowledge_base.id is not None
    assert knowledge_base.active_embedding_channel_id is not None

    async def fake_load_runtime_config(*_args, **_kwargs):
        return SimpleNamespace(declared_dimensions=3)

    async def failed_detect_dimensions(_config):
        raise RuntimeError("probe failed")

    monkeypatch.setattr(embedding_migration_module, "load_embedding_runtime_config", fake_load_runtime_config)
    monkeypatch.setattr(embedding_migration_module, "detect_embedding_dimensions", failed_detect_dimensions)
    ...
    async with migration_database() as db:
        with pytest.raises(ParameterException) as exc_info:
            await embedding_migration_module.submit_user_knowledge_base_embedding_migration(
                ...
                uid=knowledge_base.uid,
                knowledge_base_id=knowledge_base.id,
                target_channel_id=knowledge_base.active_embedding_channel_id,
                target_model_id="embedding-v2",
            )
        current = await db.get(KnowledgeBase, knowledge_base.id)
        jobs = list((await db.execute(select(KnowledgeJob))).scalars().all())
...
@pytest.mark.asyncio
async def test_user_migration_rejects_embedding_config_change_during_probe_without_job(
    migration_database: async_sessionmaker[AsyncSession],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    knowledge_base = await _create_container(migration_database)
    ...
    async def changing_load_runtime_config(*_args, **_kwargs):
        nonlocal load_count
        load_count += 1
        return SimpleNamespace(declared_dimensions=3 if load_count == 1 else 4)

    async def fake_detect_dimensions(_config):
        return 3

    monkeypatch.setattr(embedding_migration_module, "load_embedding_runtime_config", changing_load_runtime_config)
    monkeypatch.setattr(embedding_migration_module, "detect_embedding_dimensions", fake_detect_dimensions)
...
@pytest.mark.asyncio
async def test_user_migration_cleanup_failure_blocks_new_submission_without_job(
    migration_database: async_sessionmaker[AsyncSession],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    knowledge_base = await _create_container(migration_database)
    ...
    async with migration_database() as db:
        current = await db.get(KnowledgeBase, knowledge_base.id)
        assert current is not None
        current.old_collection_cleanup_status = KnowledgeBaseOldCollectionCleanupStatus.FAILED
        await db.commit()
