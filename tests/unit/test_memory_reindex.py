...
async def test_submit_memory_reindex_rejects_active_organization_chain(
    ...
    uid = f"memory_reindex-reindex-organization-block-{operation.value}"
    await configure_store(
        memory_session_factory,
        uid=uid,
        channel_id=31,
        model_id="memory_reindex-organization-block-model",
        dimensions=3,
        signature="memory_reindex-organization-block-signature",
        active_revision=2,
        index_revision=4,
        collection_name=f"memory_reindex-organization-block-{operation.value}",
    )
    ...
    async with memory_session_factory() as db:
        active_job, created = await memory_job_crud.create(
            ...
            uid=uid,
            operation=operation,
            dedupe_key=f"memory_reindex-active-organization-{operation.value}",
            payload=payload,
            commit=True,
        )
    assert created
    ...
    async with memory_session_factory() as db:
        with pytest.raises(MemoryConflictError) as exc_info:
            await submit_memory_reindex(
                db,
                uid=uid,
                dedupe_key=f"memory_reindex-blocked-reindex-{operation.value}",
            )
...
async def test_submit_memory_reindex_does_not_block_on_unrelated_delete_cleanup(
    ...
    uid = "memory_reindex-reindex-unrelated-delete-cleanup"
    await configure_store(
        memory_session_factory,
        uid=uid,
        channel_id=32,
        model_id="memory_reindex-unrelated-cleanup-model",
        dimensions=3,
        signature="memory_reindex-unrelated-cleanup-signature",
        active_revision=2,
        index_revision=4,
        collection_name="memory_reindex-unrelated-cleanup",
    )
    ...
    async with memory_session_factory() as db:
        cleanup_job, created = await memory_job_crud.create(
            ...
            dedupe_key="memory_reindex-unrelated-delete-cleanup-job",
    ...
    async with memory_session_factory() as db:
        submission = await submit_memory_reindex(
            db,
            uid=uid,
            dedupe_key="memory_reindex-reindex-with-unrelated-cleanup",
        )
...
async def test_memory_reindex_builds_switches_and_cleans_old_collection(
    ...
    uid = "memory_reindex-complete-user"
    old_collection = "memory_reindex-complete-old"
    channel_id = 4
    model_id = "memory_reindex-complete-model"
    dimensions = 3
    signature = "memory_reindex-complete-signature"
    ...
    async with memory_session_factory() as db:
        submission = await submit_memory_reindex(
            db,
            uid=uid,
            dedupe_key="memory_reindex-complete-dedupe",
        )
    ...
    result = await executor.execute_claimed(claimed, "memory-reindex-worker")
...
async def test_memory_reindex_rejects_corrupted_nonfirst_target_embedding(
    ...
    uid = "memory_reindex-corrupted-nonfirst-vector-user"
    old_collection = "memory_reindex-corrupted-nonfirst-vector-old"
    channel_id = 4
    model_id = "memory_reindex-corrupted-nonfirst-vector-model"
    dimensions = 3
    signature = "memory_reindex-corrupted-nonfirst-vector-signature"
    embedding_revision = 5
    index_revision = 8
    await configure_store(
        memory_session_factory,
    ...
    async with memory_session_factory() as db:
        submission = await submit_memory_reindex(
            db,
            uid=uid,
            dedupe_key="memory_reindex-corrupted-nonfirst-vector-dedupe",
        )
    assert submission.job.id is not None
    target_collection = submission.job.payload["target"]["collection"]
    ...
    with pytest.raises(MemoryJobRetryableError):
        await executor.execute_claimed(claimed, "memory-reindex-worker")
...
async def test_memory_reindex_accepts_small_target_embedding_difference(
    ...
    uid = "memory_reindex-small-vector-difference-user"
    old_collection = "memory_reindex-small-vector-difference-old"
    channel_id = 4
    model_id = "memory_reindex-small-vector-difference-model"
    dimensions = 3
    signature = "memory_reindex-small-vector-difference-signature"
    embedding_revision = 5
    index_revision = 8
    await configure_store(
        memory_session_factory,
    ...
    async with memory_session_factory() as db:
        submission = await submit_memory_reindex(
            db,
            uid=uid,
            dedupe_key="memory_reindex-small-vector-difference-dedupe",
        )
    assert submission.job.id is not None
    target_collection = submission.job.payload["target"]["collection"]
...
async def test_memory_reindex_does_not_persist_progress_after_lease_loss(
    ...
    uid = "memory_reindex-lease-user"
    old_collection = "memory_reindex-lease-old"
    target_collection = "memory_reindex-lease-target"
    channel_id = 5
    model_id = "memory_reindex-lease-model"
    dimensions = 3
    signature = "memory_reindex-lease-signature"
    ...
    claimed = await claim_job(
        ...
        uid=uid,
        operation=LongTermMemoryMutationOperation.REINDEX,
        dedupe_key="memory_reindex-lease-dedupe",
        owner="memory_reindex-lease-worker",
        payload=payload,
    )
    assert claimed is not None
    ...
    with pytest.raises(MemoryJobLeaseLostError):
        await executor.execute_claimed(claimed, "memory_reindex-lease-worker")
...
async def test_memory_reindex_switch_is_fenced_after_expired_lease_recovery(
    ...
    uid = "memory_reindex-reindex-switch-fence-user"
    old_collection = "memory_reindex-reindex-switch-fence-old"
    channel_id = 6
    model_id = "memory_reindex-reindex-switch-fence-model"
    dimensions = 3
    signature = "memory_reindex-reindex-switch-fence-signature"
    embedding_revision = 4
    index_revision = 10
    owner = "memory_reindex-reindex-switch-fence-worker"
    await configure_store(
        memory_session_factory,
        uid=uid,
        channel_id=channel_id,
    ...
    async with memory_session_factory() as db:
        submission = await submit_memory_reindex(
            db,
            uid=uid,
            dedupe_key="memory_reindex-reindex-switch-fence",
        )
        assert submission.job.id is not None
        await db.execute(
            update(LongTermMemoryMutationJob)
    ...
    async def recover_expired_claim() -> None:
        ...
        async with memory_session_factory() as recovery_db:
            ...
            recovery = await memory_job_crud.recover_expired(
                recovery_db,
                max_attempts_error="memory_reindex lease expired",
                commit=False,
            )
