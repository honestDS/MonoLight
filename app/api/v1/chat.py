...
async def chat_completions(request: ChatCompletionRequest, db: AsyncSession = Depends(get_db), current_user: dict = Depends(get_current_user)):
    # 异步等待调度器返回真实推理结果
    session_id = request.session_id or str(uuid.uuid4())
    llm_response = await ChatDispatcher.dispatch(db, request.message, uid=current_user['uid'], session_id=session_id)
    # 格式化输出
...
async def get_user_sessions(
    ...
    stmt = stmt.group_by(Message.session_id, Message.uid, User.username).order_by(desc('last_active'))
