...
from app.core.exceptions import ForbiddenException, ResourceNotFoundException, ParameterException
...
async def update_user(
    ...
    if user.is_superuser:
        ...
        if is_renaming or is_deactivating or is_demoting:
            raise ParameterException("超级管理员账户受核心保护，严禁执行禁用、降权或改名操作。")    
    if user_in.password:
        try:
...
async def delete_user(
    ...
    if user.is_superuser:
        raise ParameterException("禁止删除超级管理员账户。如需注销，请先通过数据库手动降权或联系系统维护员。")
