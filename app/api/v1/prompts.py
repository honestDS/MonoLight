...
async def create_prompt(data: PromptCreate, db: AsyncSession = Depends(get_db)):
    ...
    return StandardResponse.success(data=PromptResponse.model_validate(db_prompt), message=constants.MSG_PROMPT_CREATED)
...
async def update_prompt(prompt_id: int, data: PromptUpdate, db: AsyncSession = Depends(get_db)):
    ...
    await db.commit()
    await db.refresh(db_prompt)
    return StandardResponse.success(data=PromptResponse.model_validate(db_prompt), message=constants.MSG_PROMPT_UPDATED)

@router.post('/delete')
async def delete_prompt(prompt_id: int, db: AsyncSession = Depends(get_db), admin: dict = Depends(check_admin_privilege)):
    db_prompt = await db.get(PromptLibrary, prompt_id)
    if not db_prompt:
        raise ResourceNotFoundException(constants.ERR_PROMPT_NOT_FOUND)
