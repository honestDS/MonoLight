...
async def create_provider(provider_in: ProviderCreate, db: AsyncSession = Depends(get_db)):
    ...
    return StandardResponse.success(ProviderRead.model_validate(db_obj), message=constants.MSG_PROVIDER_CREATED)

@router.get('/list', response_model=StandardResponse)
async def list_providers(db: AsyncSession = Depends(get_db)):
    result = await db.execute(select(ModelProvider))
    return StandardResponse.success([ProviderRead.model_validate(item) for item in result.scalars().all()])
...
async def update_provider(provider_id: int, provider_in: ProviderUpdate, db: AsyncSession = Depends(get_db)):
    ...
    await db.commit()
    await db.refresh(db_obj)
    return StandardResponse.success(ProviderRead.model_validate(db_obj), message=constants.MSG_PROVIDER_UPDATED)
