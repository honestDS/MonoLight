...
async def login(request: LoginRequest, db: AsyncSession = Depends(get_db)):
    ...
    return StandardResponse.success({'access_token': access_token, 'token_type': 'bearer'}, message=constants.MSG_LOGIN_SUCCESS)
...
async def reset_admin_account(request: ResetAdminRequest, db: AsyncSession = Depends(get_db)):
    ...
    return StandardResponse.success(data=user_data, message="超级管理员账户信息已成功重置，请及时登录并修改默认密码。")
