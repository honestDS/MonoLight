...
MIGRATION_ID = "20260709_add_chat_session_reply_target_source"
...
async def migrate(session: AsyncSession) -> None:
    ...
    await ensure_column(session, "chat_session", "reply_target_source")
    await ensure_index(session, "chat_session", "ix_chat_session_reply_target_source", "reply_target_source")

    await session.execute(
        text(
            """
            UPDATE chat_session
            SET reply_target_source = source
            WHERE reply_target_source IS NULL OR reply_target_source = ''
            """
        )
    )
    await session.execute(
        text(
            """
            UPDATE chat_session
            SET reply_target_source = 'weixin-openclaw'
            WHERE session_id LIKE 'weixin-openclaw:%'
              AND (reply_target_source IS NULL OR reply_target_source = '' OR reply_target_source = 'http')
            """
        )
    )
