# MonoLight

MonoLight 是一个旨在重塑人机交互形态的通用自主智能体（General Autonomous Agent）核心基座。

## 我们的愿景

我们致力于构建一个极致精简且具备高度自主执行能力的底层框架。在集成多模态大模型适配、循环反思决策、物理指令审计、RAG 知识检索、动态 Skill 插件及定时任务等核心 Agentic 功能之上，MonoLight 的终极目标是成为支撑全场景外部接入的统一智能基座。

通过框架内置的多协议适配层，MonoLight 将作为逻辑中枢，深度赋能包括但不限于以下领域：
一 智能交互：构建具备长效记忆与个性化偏好的新一代聊天机器人。
二 自动化运维：实现服务器集群的自主巡检、故障自愈与云原生环境的敏捷开发。
三 具身智能：作为云端大脑节点，赋予机器人、无人设备等物理实体逻辑思考与现实任务执行的统一能力。

我们的图景是打破数字世界与物理世界的隔阂，让 MonoLight 成为连接人类意图与现实执行的通用桥梁。

---

## AI Agent 开发规范守则

> [!IMPORTANT]
> 所有参与本项目贡献的 AI Agent 必须严格遵守以下开发标准与架构原则：
> 1. 阅读并执行 [DEVELOPMENT_GUIDE.md](./DEVELOPMENT_GUIDE.md) 中的命名规范、代码风格（Ruff）及测试要求。
> 2. 参考 [ARCHITECTURE.md](./ARCHITECTURE.md) 以确保符合系统设计与模块依赖关系。
> 2. 在提交任何代码前，必须确保通过 `ruff check` 与 `ruff format` 检查。
> 3. 所有测试用例的编写或修改必须严格基于目标代码的实际实现。在编写测试前，AI Agent 必须完整阅读并解析目标源码，确保 Mock 逻辑与业务流转完全对齐，严禁凭经验或假设编写测试代码。

## 1. 核心特性
- **多类型数据库支持**: 支持sqlite(默认)、mysql、postgresql数据库。
- **自主 Agent 架构**: 内置 Dispatcher 循环机制，支持 AI 自主调用工具链完成复杂逻辑。
- **多用户系统**: 引入完整的用户权限体系与多角色管理。
- **多用户会话隔离**: 实现用户会话隔离。
- **配置管理中心**: 前后端深度打磨的配置管理页面，支持可视化管理模型参数、采样策略及提供商映射。
- **异常驱动架构**: 引入 StandardResponse 与业务异常体系，通过全局 Handler 实现精准的错误分级与业务文案收口。
- **提示词资产管理**: 独立的 PromptLibrary 支持多套提示词随时切换与复用。
- **物理工具集成**: 首发支持 ShellExecutor，允许 AI 在受控环境下执行系统指令。
- **全异步架构**: 基于 FastAPI (aiohttp) 与 SQLAlchemy 的高性能并发处理。

## 已知限制
- 连续对话以上一轮 API 返回的真实 input token 数为基线，增量估算下一轮；运行时提示仅附加到每轮最新用户消息，跨轮时会从旧消息移到新消息，因此下一轮发送前的增量估算会保留上一轮提示的 token，可能略微高估并提前触发上下文总结。真实模型响应返回 usage 后，界面展示会由实际值校正；当前暂不通过额外探测请求校准，以避免额外 token 消耗。

## 2. 交互入口
- **仪表盘 (Dashboard)**: 基于 Vue 3 + Element Plus 的现代管理后台，提供极致流畅的配置与交互体验。
- **API 文档**: 内置 Swagger (/docs)，支持标准的鉴权与业务接口调用。

## 3. 未来规划
- [x] 完成核心配置及WEB对话流程(HTTP)
- [x] 完成会话历史记录接口
- [x] 解耦dashboard的所有.vue单文件 将逻辑于模板分离
- [x] 完成前端webscoket对话流程
- [x] 完成后端流式请求支持 实现真正的流式对话
- [x] 会话消息追加功能: 实现agent在处理多轮步骤过程中的消息追加功能。
- [x] 自动会话标题生成: 实现新建会话时自动根据会话内容生成标题的功能。
- [x] 实现markdown格式输出开关: 支持用户在会话中切换输出格式为Markdown或纯文本。
- [ ] **多用户SSH隔离**: 实现服务器操作环境与数据隔离。
- [x] **计划任务系统**: 支持 Cron 表达式驱动的 Agent 任务定时执行与循环触发。
- [x] **后台实时日志**: 集成 WebSocket 实现服务端执行日志的毫秒级流式下发与可视化审计。
- [x] **深度参数配置化**: 抽离核心业务逻辑参数至统一配置中心，支持热加载与多环境无缝切换。
- [ ] **Skill 动态加载**: 实现技能库的热插拔与在线热更新机制。
- [x] **嵌入模型与知识库支持**: 集成 RAG 架构，支持云端向量数据库以增强 Agent 的长效知识索引能力。
- [x] **知识库注入工具化**: 将知识库列表以系统提示词方式进行注入，使用类SKILLS的渐进式披露机制将知识库查询作为工具提供给LLM使用。
- [x] **重排序模型支持**: 引入 Rerank 机制优化知识库检索精度，显著提升 Agent 对长文本上下文的理解与过滤能力。
- [x] **更多的上下文压缩机制选择**: 引入滑动窗口、摘要压缩，在保持逻辑一致性的前提下最大限度节省 Token 消耗。
- [x] **实现图片、文件传输**: 支持用户上传图片、文件等二进制数据，实现与大模型的直接交互。
- [ ] **全模态支持**: 除了图片、文本、文件的数据传输，还支持视频、音频等多媒体数据的上传与交互。
- [ ] **接入 QQ平台 消息适配器**: 实现与 QQ平台 的消息交互功能。
- [x] **接入 微信平台 消息适配器**: 实现与 微信平台 的消息交互功能。
- [x] **模型渠道管理**: 实现多模型渠道的管理，支持自定义渠道优先级、权重、调度策略。
- [ ] **长期记忆管理**: 借助现有的关系型数据库原文存储+RAG机制，实现用户会话的长期记忆与上下文理解的稠密与稀疏混合检索。
- [ ] **用户偏好管理**: 借助现有的关系型数据库原文存储+RAG机制，实现用户级的个性化偏好录入(LLM自主录入)及混合检索。
- [ ] **企业级审计系统**: 使用平台自带的双模型审计机制，实现企业级的审计报告自动生成与发送，且在发送后立即删除暂存在服务器上的审计数据，避免被恶意篡改或泄露。

## 4. 技术架构
详细文档请参考 [ARCHITECTURE.md](./ARCHITECTURE.md)

## 5. 开源协议
本项目采用 AGPL-3.0 协议开源。

## 运行服务

项目包含 Web 服务、消息平台 Worker 和后台任务 Worker 三类进程。Web 服务支持多个 Worker，两个后台 Worker 通过数据库租约保证同一数据库范围内各自只有一个有效实例。

在 `.env` 中配置监听地址、端口和 Web Worker 数量：

```dotenv
APP_HOST=0.0.0.0
APP_PORT=8001
APP_WORKERS=1
```

推荐在 Windows、Linux 和 macOS 上使用统一启动器：

```bash
python start.py
```

启动器会先完成数据库建表、迁移和系统数据初始化，再启动 Web 服务、消息平台 Worker 和后台任务 Worker；任一子进程异常退出或收到终止信号时，启动器会关闭其余子进程。

需要分别调试各进程时，先执行一次全局初始化：

```bash
python -c "import asyncio; from start import initialize_system; asyncio.run(initialize_system())"
```

然后分别启动：

```bash
python main.py
python -m app.workers.message_platform
python -m app.workers.background_task
```

多实例部署时，所有实例必须连接同一个数据库。未取得租约的后台 Worker 会保持待命，并在当前持有者退出或租约过期后自动接管。

## 自动化测试
项目已接入自动化测试体系，涵盖单元测试、初始化逻辑测试以及 API 集成测试。

### 执行测试命令
在项目根目录下执行以下命令运行全量测试：

```bash
PYTHONPATH=. pytest tests/
```
