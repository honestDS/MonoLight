...
> [!IMPORTANT]
> 低于上述要求的模型可能仍可用于简单聊天或部分功能，但不建议用于完整的 MonoLight Agent 工作流。系统会通过历史总结、当前工具轮结果的一次性截断和模型请求前的预算校验控制上下文体积；已持久化的历史工具结果不会在后续请求中再次截断或通过滑动窗口改写；如果在旧历史已经压缩后，最近必要交互和工具结果仍无法放入模型上下文，通常意味着当前模型的上下文能力不足以承载该任务。主聊天、上下文总结、长期记忆整理和安全审计可以分别选择模型，其中主聊天、总结、记忆整理和审计当前均使用 对话用途的模型条目。
