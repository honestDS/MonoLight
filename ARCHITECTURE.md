...
该目录承载 LLM 托管知识主数据，以及托管知识与用户知识库的统一知识召回领域能力。正文和版本历史以关系库为事实源，向量仅作派生索引。首次写入按 Profile 懒创建唯一托管知识库并绑定 Profile；容器、绑定、首条知识和发布作业在同一短事务提交。托管知识召回最终由关系库版本、墓碑和 `is_recallable` 判定。

统一知识召回按当前 Profile 收集托管知识库和已绑定用户知识库，按生效嵌入配置复用查询向量，各库执行混合候选召回后合并为全局候选池，并只执行一次 Profile Reranker。托管知识的失效候选由关系库过滤，必要时逐步扩展向量候选窗口；用户文档相邻分块根据关系库保存的原文和原始分块参数恢复正文，不使用字符串重叠猜测。最终结果统一执行知识配置的候选上限、`top_k` 和字符预算，并保留来源类型及托管知识可维护边界。统一知识召回由长期记忆预检与长期记忆、聊天历史召回并发执行，三类来源分别使用自己的候选数量、最终数量和字符预算；统一知识结果写入工具响应的 `knowledge_base` 字段，不挤占长期记忆 `items` 或 `chat_history` 的额度。长期记忆关闭时，独立 `query_knowledge_base` 工具只在当前 Profile 存在已发布且索引可用的用户知识库时动态暴露，不受 `enabled_tools` 遗留值控制；托管知识库不会进入该独立工具白名单。
...
用户知识库和托管知识库共用在线嵌入迁移引擎。迁移以关系数据库完整正文为事实源，先锁定快照边界并分页构建目标集合，迁移期间的关系数据变化写入单调增量日志；目标集合通过数量、标识、元数据、维度和抽样查询校验后，复用锁外最终计划并以增量水位作写入栅栏，在短事务内批量更新必要向量引用并切换 `active_*`，事务内不再全量读取或重新切分正文。迁移最终切换与知识整理发布互斥；旧集合清理成功前禁止开始下一次嵌入迁移。取消或切换前失败不会改变当前生效集合，失败目标集合与切换后的旧集合都通过持久化 cleanup KnowledgeJob 独立清理，不由调用栈直接 best effort 删除。
...
知识作业与长期记忆作业模型独立，但复用租约、重试和旧 Worker 栅栏原则。数据库短事务只负责预留、校验和发布；嵌入与向量调用在事务外执行。所有系统 cleanup 都持久化为不可业务取消的 KnowledgeJob，可重试外部失败使用固定间隔持续重试，不受普通最大尝试次数终止；确定性载荷或契约错误才进入终态失败。Consumer 启动先恢复过期租约，续期失败或续期数据库异常都会立即停止旧执行协程。每次 Knowledge Worker 启动并完成租约恢复后，为每个知识库提交一次持久化一致性巡检；同一知识库同一时刻只允许一个活动巡检，Worker 常驻期间不按小时或按日重复调度。巡检分批清理超过 7 天的终态整理 Stage/Fragment 和孤儿向量，并在关系数据、向量引用、集合、维度或元数据不一致时提交正规重建作业。删除先墓碑化停止召回，再异步清理向量与条目。
...
`app/models/knowledge_base.py` 定义知识库容器、用户文档、托管知识、版本历史、知识作业，以及知识整理快照、阶段和片段。整理快照仅保存稳定元数据和托管知识版本引用，完整正文继续由版本历史作为不可变来源；Stage/Fragment 保存单次整理任务的稳定工作身份、模型快照、连续进度和模型结果，只允许同一任务同一 Stage 在瞬时失败重试时复用连续前缀，跨任务不得恢复或复用；父 Stage 进入终态后的完成时间超过 7 天后，相关 Stage/Fragment 才可由维护作业清理。托管条目保存正文、稳定标识、版本、来源和维护状态；知识作业保存幂等、租约、重试和目标版本，不携带知识正文；向量分块仅作派生索引。
