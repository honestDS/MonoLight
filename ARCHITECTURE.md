...
├── organization.py         # 托管知识整理快照冻结、版本引用还原与稳定工作身份
...
统一知识召回按当前 Profile 收集托管知识库和已绑定用户知识库，按生效嵌入配置复用查询向量，各库执行混合候选召回后合并为全局候选池，并只执行一次 Profile Reranker。托管知识的失效候选由关系库过滤，必要时逐步扩展向量候选窗口；用户文档相邻分块根据关系库保存的原文和原始分块参数恢复正文，不使用字符串重叠猜测。最终结果统一执行知识配置的候选上限、`top_k` 和字符预算，并保留来源类型及托管知识可维护边界。该统一知识召回服务当前保持为独立领域服务，接入长期记忆统一召回属于后续调度层步骤。
