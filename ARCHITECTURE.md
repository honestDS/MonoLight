...
用户知识库可由管理页面独立选择目标嵌入渠道和模型并提交迁移，与长期记忆开关无关；托管知识库不接受独立模型选择。长期记忆嵌入修订原子切换时，先持有该用户长期记忆状态锁，再以锁定读取获取最新已提交的全部托管知识库，并在同一事务分别提交迁移作业；托管知识库首次创建也必须先取得同一长期记忆状态锁，因此切换前已创建的容器不会因 MySQL 可重复读快照遗漏，切换期间的新建请求则会在切换提交后读取新的生效配置。作业状态按知识库隔离。
